...
export class BlogCommentCollection {
  ...
  constructor(baseQuery: any) {
    ...
    this.baseQuery.orderBy('blog_comment.created_at', 'DESC');
  }

  async init(filters: Filter[] = []): Promise<void> {
    const currentFilters: Filter[] = [];
    const blogCommentCollectionFilters = await getValue(
      'blogCommentCollectionFilters',
      []
    );
    blogCommentCollectionFilters.forEach((filter: any) => {
    ...
    totalQuery.select('COUNT(blog_comment.blog_comment_id)', 'total');
