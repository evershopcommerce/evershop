...
module.exports = exports = async (connection) => {
  ...
  await execute(
    ...
    BEGIN
      IF(NEW.url_key IS NULL) THEN
        ...
        NEW.url_key = url_key;
      ELSE
        IF (NEW.url_key !~ '^[^/\s\\#]+$') THEN
          RAISE EXCEPTION 'Invalid url_key';
        END IF;
      END IF;
      RETURN NEW;
    END;
  ...
  await execute(
    ...
    `CREATE TABLE product_inventory (
      ...
      CONSTRAINT "PRODUCT_INVENTORY_PRODUCT_ID_CONSTANTSRAINT" FOREIGN KEY ("product_id") REFERENCES "product" ("product_id") ON DELETE CASCADE,
  ...
  await execute(
    connection,
    `INSERT INTO product_inventory (product_id, qty, manage_stock, stock_availability)
