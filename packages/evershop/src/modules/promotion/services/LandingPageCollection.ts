...
/**
 * Paginated collection for landing pages (mirrors BlogPostCollection). Non-admin
 * callers only see published pages. Single table — no `_description` join.
 */
export class LandingPageCollection {
  ...
  constructor(baseQuery: any) {
    ...
    this.baseQuery.orderBy('landing_page.created_at', 'DESC');
  }

  async init(filters: Filter[] = [], isAdmin = false): Promise<void> {
    if (!isAdmin) {
      this.baseQuery.andWhere('landing_page.status', '=', true);
    }

    const currentFilters: Filter[] = [];
    const landingPageCollectionFilters = await getValue(
      'landingPageCollectionFilters',
      []
    );

    landingPageCollectionFilters.forEach((filter: any) => {
    ...
    totalQuery.select('COUNT(landing_page.landing_page_id)', 'total');
