...
*/
module.exports.buildTailwind = async function buildTailwind(routes, clientOnly = false) {
  await Promise.all(routes.map(async (route) => {
    ...
    components.forEach((module) => {
      ...
      const list = dependencyTree.toList({
        ...
        filter: path => path.indexOf('.js') !== -1 && (path.indexOf('node_modules') === -1 || path.indexOf('@evershop') !== -1),
      });
    ...
    let tailwindConfig = {};
    if (route.isAdmin) {
      if (fs.existsSync(path.join(CONSTANTS.ROOTPATH, 'tailwind.admin.config.js'))) {
        tailwindConfig = require(path.join(CONSTANTS.ROOTPATH, 'tailwind.admin.config.js'));
      }
    } else {
      if (fs.existsSync(path.join(CONSTANTS.ROOTPATH, 'tailwind.frontStore.config.js'))) {
        tailwindConfig = require(path.join(CONSTANTS.ROOTPATH, 'tailwind.frontStore.config.js'));
      }
    }
...
@tailwind utilities;`
      ...
      await writeFile(path.resolve(subPath, 'client', 'tailwind.scss'), tailwindCss);
    } catch (error) {
