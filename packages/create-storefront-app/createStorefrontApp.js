...
function init() {
  const program = new commander.Command(packageJson.name)
    ...
    .on('--help', () => {
      ...
      console.log(
        `      ${chalk.cyan(
          'https://github.com/nttare0/storefront/issues/new'
        )}`
      );
      console.log();
    })
  ...
  if (typeof projectName === 'undefined') {
    ...
    console.log(
      `  ${chalk.cyan(program.name())} ${chalk.green('my-storefront-app')}`
    );
    console.log();
    console.log(
  ...
  checkForLatestVersion()
    .catch(() => {
      try {
        return execSync('npm view create-storefront-app version')
          .toString()
          .trim();
      } catch (e) {
        return null;
    ...
    .then((latest) => {
      if (latest && semver.lt(packageJson.version, latest)) {
        ...
        console.error(
          chalk.yellow(
            `You are running \`create-storefront-app\` ${packageJson.version}, which is behind the latest release (${latest}).\n\n` +
              'We recommend always using the latest version of create-storefront-app if possible.'
          )
        );
        console.log();
        console.log(
          'The latest instructions for creating a new app can be found here:\n' +
            'https://github.com/nttare0/storefront#readme'
        );
        console.log();
      } else {
...
function createApp(name, verbose, useYarn) {
  ...
  console.log(`Creating a new Storefront app in ${chalk.green(root)}.`);
  console.log();
...
function install(root, useYarn, dependencies, verbose, isOnline) {
  return new Promise((resolve, reject) => {
    ...
    if (useYarn) {
      ...
      args = [
        'install',
        '--no-audit', // https://github.com/nttare0/storefront/issues/11174
        '--save',
        '--save-exact',
        '--loglevel',
        'error'
...
function run(root, appName, verbose, originalDirectory, useYarn) {
  console.log(`Installing ${chalk.cyan('@storefront/core')}`);
  checkIfOnline(useYarn)
    .then((isOnline) => ({
      isOnline
    }))
    .then(({ isOnline }) => {
      const allDependencies = ['@storefront/core'];
...
// https://github.com/nttare0/storefront/pull/368#issuecomment-243446094
...
// See https://github.com/nttare0/storefront/pull/3355
...
async function setUpStorefront(projectDir) {
...
function checkForLatestVersion() {
  return new Promise((resolve, reject) => {
    https
      .get(
        'https://registry.npmjs.org/-/package/create-storefront-app/dist-tags',
        (res) => {
