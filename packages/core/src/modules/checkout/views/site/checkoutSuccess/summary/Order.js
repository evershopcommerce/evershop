...
const OrderSummary = () => {
    ...
    const _shippingCost = new Intl.NumberFormat(language, { style: 'currency', currency: currency }).format(order.shipping_fee_excl_tax);
    const _taxAmount = new Intl.NumberFormat(language, { style: 'currency', currency: currency }).format(order.tax_amount);
    const _discountAmount = new Intl.NumberFormat(language, { style: 'currency', currency: currency }).format(order.discount_amount);
    const _subTotal = new Intl.NumberFormat(language, { style: 'currency', currency: currency }).format(order.sub_total);
    const _grandTotal = new Intl.NumberFormat(language, { style: 'currency', currency: currency }).format(order.grand_total);
