          ...
          <tbody>
            {
              items.map((item, index) => {
                const _regularPrice = new Intl.NumberFormat(language, { style: 'currency', currency }).format(item.product_price);
                const _finalPrice = new Intl.NumberFormat(language, { style: 'currency', currency }).format(item.final_price);
                  ...
                  <tr key={index}>
                    <td>
                      <div className="flex justify-start space-x-1 product-info">
                        <div className="flex justify-center">
                          ...
                          {!item.thumbnail && (
                            <svg className="self-center" xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <td>
                      {parseFloat(item.final_price) < parseFloat(item.product_price) && (
                        <div>
                          <span className="regular-price">{_regularPrice}</span>
                          {' '}
