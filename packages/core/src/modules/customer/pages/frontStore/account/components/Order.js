...
export default function Order({ order }) {
  return <div className='order border-divider'>
    <div className='order-inner grid grid-cols-1 md:grid-cols-3 gap-2'>
      <div className='order-items col-span-2'>
        {order.items.map((item) => <div className='order-item mb-1 flex gap-2 items-center'>
          ...
          <div className='order-item-info'>
            ...
            <div className='order-item-sku italic'>Sku: #{item.productSku}</div>
            <div className='order-item-qty' style={{ fontSize: '0.9em' }}>{item.qty} x {item.productPrice.text}</div>
          </div>
        </div>)}
      </div>
      <div className='order-total col-span-1'>
        <div className='order-header'>
          <div className='order-number'><span className='font-bold'>Order: #{order.orderNumber}</span><span className='italic pl-1'>{order.createdAt.text}</span></div>
        </div>
        <div className='order-total-value font-bold'>Total: {order.grandTotal.text}</div>
      </div>
    </div>
  </div>
}