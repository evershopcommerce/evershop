      ...
      {attributes.map((a, i) => (
        <div key={i}>
          ...
          <ul className="flex flex-wrap filter-option-list">
            {a.options.map((o, j) => {
              const isChecked = activeFilters.find((f) => f.key === a.attribute_code && parseInt(f.value) === parseInt(o.option_id));
              return (
                <li key={j} className="mt-05 mr-05">
                  <a
                    href="#"
      ...
      <div className="page-width flex justify-between">
        <div className="self-center">
          <div className="filter-heading">
            {filtering === false && (
              <a href="#" className="flex justify-start space-x-05" onClick={(e) => { e.preventDefault(); setFiltering(true); }}>
