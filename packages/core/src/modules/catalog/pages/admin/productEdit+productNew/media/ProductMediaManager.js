...
function Upload({ addImage, productImageUploadUrl }) {
  const onChange = (e) => {
    ...
    fetch(
      productImageUploadUrl + targetPath,
      {
        method: 'POST',
        body: formData,
        headers: {
...
function Images({
  id, images, addImage, removeImage, productImageUploadUrl
}) {
  return (
    <div id={id} className="image-list">
      {images.map((image) => <Image key={image.id} removeImage={removeImage} image={image} />)}
      <Upload addImage={addImage} productImageUploadUrl={productImageUploadUrl} />
    </div>
  );
}
