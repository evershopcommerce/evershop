...
module.exports = (request, response, stack, next) => {
  ...
  const payload = Object.assign({ customerId: null, cartId: null, customerEmail: null, sid: uuidv4() }, getContextValue(request, 'tokenPayload', {}));
  signToken(
    payload,
    { expiresIn: '2d' },
    (newToken) => {
      response.cookie(getTokenCookieId(), newToken, { maxAge: 172800000, httpOnly: true });
      setContextValue(request, 'token', newToken);
    }
  );
  next();
};
