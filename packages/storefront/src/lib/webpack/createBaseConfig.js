...
export function createBaseConfig(isServer) {
  ...
  // Resolve aliases
  const alias = {
    '@storefront/core/components': path.resolve(__dirname, '../../components')
  };
  if (theme) {
  ...
  if (
    !isRealDirectorySync(
      path.resolve(CONSTANTS.ROOTPATH, 'node_modules', '@storefront', 'core')
    )
  ) {
    alias['@storefront/core'] = path.resolve(
      CONSTANTS.ROOTPATH,
      'packages',
      'storefront',
      'dist'
