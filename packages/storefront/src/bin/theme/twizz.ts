...
async function scanModulesFrontStore(): Promise<string[]> {
  ...
  const modulesDir = (await isRealDirectory(storefrontDir))
    ? path.join(storefrontDir, 'src', 'modules')
    : path.join(process.cwd(), 'packages', 'storefront', 'src', 'modules');
  let results: string[] = [];
  try {
...
async function getOverrideCandidates(): Promise<string[]> {
  // Check if a folder @storefront/core exists in the node_modules
  const storefrontDir = path.join(
    process.cwd(),
    'node_modules',
    '@storefront',
  ...
  if (await isRealDirectory(storefrontDir)) {
    commonDir = path.join(storefrontDir, 'src', 'components', 'common');
    frontStoreDir = path.join(storefrontDir, 'src', 'components', 'frontStore');
  } else {
    commonDir = path.join(
      process.cwd(),
      'packages',
...
async function createOverrideFile() {
  ...
  const baseDir = (await isRealDirectory(storefrontDir))
    ? path.join(storefrontDir, 'src')
    : path.join(process.cwd(), 'packages', 'storefront', 'src');
