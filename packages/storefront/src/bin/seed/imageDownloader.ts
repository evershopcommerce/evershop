...
/**
 * Copy a bundled seed asset (a path relative to the project root) to the
 * destination. Used so seeding works fully offline.
 */
function copyLocalImage(source: string, outputPath: string): string {
  const absoluteSource = isAbsolute(source)
    ? source
    : resolvePath(CONSTANTS.ROOTPATH, source);

  if (!existsSync(absoluteSource)) {
    throw new Error(`Seed image not found: ${absoluteSource}`);
  }
  ...
  copyFileSync(absoluteSource, outputPath);
  info(`  ✓ Copied: ${absoluteSource} → ${outputPath}`);
  return outputPath;
}

/**
 * Download an image from a URL - or copy it from disk when the source is a
 * local, bundled seed asset - and save it to a local file.
 */
export async function downloadImage(
  ...
  if (!/^https?:\/\//i.test(url)) {
    return copyLocalImage(url, outputPath);
  }
