...
export async function buildEntry(routes, clientOnly = false) {
  ...
  await Promise.all(
    routes.map(async (route) => {
      ...
      import { Area } from '@storefront/core/components/common';
      import {${
        route.isAdmin ? 'HydrateAdmin' : 'HydrateFrontStore'
      }} from '@storefront/core/components/common';
      `;
      areas['*'] = areas['*'] || {};
      ...
      if (!clientOnly) {
        ...
        contentServer += '\r\n';
        contentServer += `import { Area } from '@storefront/core/components/common';`;
        contentServer += '\r\n';
        contentServer += `import { renderHtml } from '@storefront/core/components/common';\r\n`;
