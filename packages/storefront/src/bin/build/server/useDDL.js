...
if (existsSync(path.resolve(CONSTANTS.ROOTPATH, './.storefront/build'))) {
  rmdirSync(path.resolve(CONSTANTS.ROOTPATH, './.storefront/build'), {
    recursive: true
  });
}
...
webpackVendorPromise.then(async () => {
  controllers.forEach((route) => {
    const buildFunc = async function () {
      ...
      content += '\r\n';
      await mkdir(
        path.resolve(CONSTANTS.ROOTPATH, './.storefront/build', buildPath),
        { recursive: true }
      );
      await writeFile(
        path.resolve(
        ...
        plugins: [
          new webpack.DllReferencePlugin({
            manifest: path.resolve(
              CONSTANTS.ROOTPATH,
              './.storefront/build/vendor-manifest.json'
            )
          })
        ]
      });
