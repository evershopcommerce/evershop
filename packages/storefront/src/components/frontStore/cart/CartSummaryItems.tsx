      ...
      {Array.from({ length: rows }).map((_, i) => (
        <li key={i} className="flex items-center py-6 animate-pulse">
          <div className="relative mr-4">
            ...
            <span className="absolute -top-2 -right-2 bg-surfaceSubdued rounded-full w-6 h-6 flex items-center justify-center text-textSubdued text-sm">
              {i + 1}
            </span>
          </div>
          <div className="flex-1 min-w-0 items-start align-top">
            <div className="h-4 bg-gray-200 rounded w-3/5 mb-2" />
            <div className="h-3 bg-surfaceSubdued rounded w-2/5 mb-1" />
          </div>
...
const CartSummaryItemsList: React.FC<{
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-8 text-textSubdued">
      ...
      {items.map((item) => (
        <li key={item.uuid} className="flex items-start py-3">
          <div className="relative mr-4 self-center">
            ...
            <span className="absolute -top-2 -right-2 bg-gray-200 rounded-full w-6 h-6 flex items-center justify-center text-secondary text-sm">
              {item.qty}
            </span>
          </div>
          <div className="flex-1 min-w-0 items-start align-top">
