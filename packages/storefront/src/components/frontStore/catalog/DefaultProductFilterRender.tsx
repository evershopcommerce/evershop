  ...
  return (
    <ProductFilterDispatch.Provider value={contextValue}>
      <button
        ...
        className="md:hidden w-full flex items-center justify-center space-x-2 py-3 px-4 border border-border rounded-xl bg-white hover:bg-surfaceSubdued transition-colors"
      >
        <svg
          className="w-5 h-5"
          fill="none"
      ...
      {isMobileFilterOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-primary/40 backdrop-blur-sm flex items-end">
          <div className="w-full bg-white rounded-t-lg max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b">
              ...
              <button
                ...
                className="p-2 hover:bg-surfaceSubdued rounded-full"
              >
                <svg
                  className="w-6 h-6"
                  fill="none"
            ...
            <div className="p-4 border-t flex space-x-3">
              <button
                ...
                className="flex-1 py-2 px-4 border border-border rounded-xl text-secondary hover:bg-surfaceSubdued disabled:opacity-50"
              >
                {_('Clear All')}
              </button>
              <button
                onClick={() => setIsMobileFilterOpen(false)}
                className="flex-1 py-2 px-4 bg-primary text-white rounded-xl hover:bg-primaryHover"
              >
                {_('Apply Filters')}
              </button>
            </div>
        ...
        <div className="product__filters__header flex items-center justify-between mb-4">
          {title && (
            <h3 className="font-bold text-lg flex items-center">
              {title}
              {activeFilterCount > 0 && (
                <span className="ml-2 bg-accentSoft text-accent text-sm px-2 py-1 rounded-full">
                  {activeFilterCount}
                </span>
              )}
              {isLoading && (
                <div className="ml-2 w-4 h-4 border-2 border-accent border-t-transparent rounded-full animate-spin"></div>
              )}
            </h3>
          )}
          ...
          {activeFilterCount > 0 && (
            <button
              onClick={clearAllFilters}
              disabled={isLoading}
              className="text-sm text-textSubdued hover:text-red-600 transition-colors disabled:opacity-50"
            >
              {_('Clear All')}
            </button>
          )}
