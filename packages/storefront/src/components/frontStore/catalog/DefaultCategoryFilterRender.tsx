    ...
    <div className="category__filter__section border-b border-gray-200 pb-2 mb-2">
      <div className="filter__header flex items-center justify-between mb-3">
        <button
          ...
          className="flex items-center space-x-2 text-left flex-1 hover:text-accent transition-colors"
        >
          <span className="font-medium">Categories</span>
          {selectedCount > 0 && (
            <span className="bg-accentSoft text-accent text-xs px-2 py-1 rounded-full">
              {selectedCount}
            </span>
          )}
          <svg
        ...
        {selectedCount > 0 && (
          <button
            ...
            className="text-textSubdued hover:text-red-500 text-sm transition-colors"
        ...
        <div className="filter__content">
          {categories.length > 5 && (
            <div className="mb-3">
              <input
                ...
                className="w-full px-3 py-2 border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-accent"
              />
            </div>
          )}
          ...
          <div className="category__options space-y-2 max-h-48 overflow-y-auto">
            {filteredCategories.length > 0 ? (
              filteredCategories.map((category) => {
                ...
                return (
                  <label
                    ...
                    className={`flex items-center space-x-3 cursor-pointer p-2 rounded hover:bg-surfaceSubdued transition-colors ${
                      isSelected ? 'bg-accentSoft border border-decorative' : ''
                    }`}
                  >
                    <input
                      type="checkbox"
              ...
              <div className="text-textSubdued text-sm text-center py-4">
