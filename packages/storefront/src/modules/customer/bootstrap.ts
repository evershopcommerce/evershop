...
export default () => {
   ...
   * @param {*} callback
   */
  (request as StorefrontRequest).loginCustomerWithEmail = async function login(
    email,
    password,
    callback
  ) {
  ...
  (request as StorefrontRequest).logoutCustomer = function logout(callback) {
  ...
  (request as StorefrontRequest).isCustomerLoggedIn =
    function isCustomerLoggedIn() {
      return !!this.session?.customerID;
    };

  (request as StorefrontRequest).getCurrentCustomer =
