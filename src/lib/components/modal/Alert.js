...
const Alert = ({ children }) => {
    ...
    const openAlert = ({ heading, content, primaryAction, secondaryAction }) => {
        dispatchAlert({ type: 'open', payload: { heading, content, primaryAction, secondaryAction } });
        dispatch({ type: 'open' });
    }

    return <AlertContext.Provider value={{ dispatchAlert: dispatchAlert, openAlert, closeAlert: () => dispatch({ type: 'closing' }) }}>
        {children}
        {state.showing === true && ReactDOM.createPortal(<div className={state.closing === false ? "modal-overlay fadeIn" : "modal-overlay fadeOut"} onAnimationEnd={(e) => { if (state.closing) { dispatch({ type: 'close' }); dispatchAlert({ type: 'remove' }); } }}>
                    ...
                    <Card title={alert.heading}>
                        ...
                        <Card.Session>
                            <div className='flex justify-end space-x-1'>
                                {alert.primaryAction && <Button
                                    {...alert.primaryAction}
                                />}
                                {alert.secondaryAction && <Button
