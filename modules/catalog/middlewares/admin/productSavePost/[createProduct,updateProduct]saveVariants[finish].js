const { insert, select, update, insertOnUpdate, del } = require('@nodejscart/mysql-query-builder');
...
module.exports = async (request, response, stack) => {
    ...
    let product = await select().from("product").where("product_id", "=", productId).load(connection);

    // If "variant_group" is not available, do nothing
    if (request.body.variant_group === undefined)
        return;

    // No variants
    if (request.body.variant_group.variants === undefined || request.body.variant_group.variants.length === 0) {
    ...
    if (!request.body.variant_group.variant_group_attributes || Object.keys(request.body.variant_group.variant_group_attributes).length === 0)
        return
    ...
    if (attributes.length < request.body.variant_group.variant_group_attributes.length)
        return;
    ...
    await Promise.all(attributes.map((a) => (async () => {
        await insertOnUpdate("attribute_group_link")
            ...
            .prime("group_id", parseInt(request.body.group_id))
            .execute(connection);
    })()));

    // Maximum 5 attributes
    await del("variant_group").where("variant_group_id", "=", parseInt(`0${product["variant_group_id"]}`)).execute(connection);
    let variantId = get(await insert("variant_group")
        .given({
            attribute_group_id: parseInt(request.body.group_id),
    ...
    let variants = request.body.variant_group.variants;
    ...
    await Promise.all(Object.keys(variants).map(k => (async () => {
        {
            ...
            if (v.sku === product.sku) {
                ...
                let options = await Promise.all(v.attributes.map((a) => (async () => {
                    return await select()
...
async function saveVariant(mainProductId, variantProductId, variantSku, variantPrice, variantQty, variantVisibility, variantAttributes, variantImages = [], connection) {
    ...
    await Promise.all(attributes.map(a => (async () => {
        {
            let va = variantAttributes.find(va => parseInt(va.attribute_id) === parseInt(a.attribute_id));
            if (va) {
