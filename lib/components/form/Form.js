    ...
    return (
        ...
        >
            <form ref={formRef} id={id} action={action} method={method} onSubmit={(e) => onSubmit(e)}>
                {children}
                {submitBtn === true && <div className={"form-submit-button"}><a className="" href="#" onClick={(e) => { e.preventDefault(); formRef.current.dispatchEvent(new Event('submit', { cancelable: true, bubbles: true })) }}>{(btnText || "Submit")}</a></div>}
            </form>
        </formContext.Provider>
    );
}
